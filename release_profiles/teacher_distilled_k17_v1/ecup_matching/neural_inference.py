...
def _score_neural_pairs(
    *,
    ...
    def serialize_chunk(start: int) -> tuple[np.ndarray, list[str], list[str] | None]:
        ...
        if bundle.serialization_mode == "pair_v2":
        ...
        # Preserve the evaluator's sequential batch composition. BF16/SDPA logits can drift
        # enough to perturb close ranks when the same rows are regrouped by approximate length.
