...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--replay-train", type=Path)
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if not 0.0 <= args.replay_fraction < 0.5:
        raise ValueError("replay_fraction must be in [0, 0.5)")
    if (args.replay_train is None) != (args.replay_fraction == 0.0):
        raise ValueError(
            "replay_train must be provided exactly when replay_fraction is positive"
        )
    if args.replay_fraction and args.loss_mode != "row_mean":
        raise ValueError("mixed-domain replay currently requires row_mean loss")
    if args.replay_fraction and args.hard_replay_fraction:
        raise ValueError("mixed-domain replay and hard replay are mutually exclusive")
    ...
    if args.replay_train is not None and not args.replay_train.is_file():
        raise FileNotFoundError(args.replay_train)
...
def _save_resume_checkpoint(
    *,
    ...
    payload = {
        ...
        "rows_seen": rows_seen,
        "primary_rows_seen": primary_rows_seen,
...
def main() -> None:
    ...
    planned_replay_rows = 0
    replay_batch_size = 0
    if args.replay_train is not None:
        replay_available_rows = parquet_rows(args.replay_train)
        replay_batch_size = replay_rows_for_primary(args.batch_size, args.replay_fraction)
        full_batches, remainder = divmod(train_rows, args.batch_size)
        planned_replay_rows = full_batches * replay_batch_size
        if remainder:
            planned_replay_rows += replay_rows_for_primary(remainder, args.replay_fraction)
        if replay_available_rows < planned_replay_rows:
            raise ValueError(
                "replay dataset is too small for one primary epoch: "
                f"available={replay_available_rows} required={planned_replay_rows}"
            )
    ...
    resume_primary_rows_seen = (
        int(resume_state.get("primary_rows_seen", resume_state["rows_seen"]))
        if resume_state is not None
        else 0
    )
    resume_replay_rows_seen = (
        int(resume_state.get("replay_rows_seen", 0)) if resume_state is not None else 0
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        primary_rows_seen = resume_primary_rows_seen if resuming_epoch else 0
        replay_rows_seen = resume_replay_rows_seen if resuming_epoch else 0
        optimizer.zero_grad(set_to_none=True)
        primary_batches = iter_pair_batches(
            args.train,
        ...
        if args.replay_train is not None:
            replay_batches = iter_pair_batches(
                args.replay_train,
                batch_size=replay_batch_size,
                read_batch_size=max(args.read_batch_size, replay_batch_size),
                max_attribute_characters=args.max_attribute_characters,
                ...
                pair_swap_probability=args.pair_swap_probability,
                serialization_mode=args.serialization_mode,
                max_rows=planned_replay_rows,
            )
            batches = iter_mixed_pair_batches(
                primary_batches,
                replay_batches,
                replay_fraction=args.replay_fraction,
            )
        else:
            batches = ((batch, len(batch), 0) for batch in primary_batches)
        if processed_batches:
            batches = islice(batches, processed_batches, None)
        if not args.no_prefetch:
            batches = prefetch_batches(batches)
        for batch_index, (batch, primary_batch_rows, replay_batch_rows) in enumerate(
            batches,
            ...
            global_step += 1
            rows_seen += len(batch)
            primary_rows_seen += primary_batch_rows
            replay_rows_seen += replay_batch_rows
            ...
            if batch_index % args.log_every == 0:
                ...
                print(
                    ...
                    f"primary_rows={primary_rows_seen}/{train_rows} "
                    f"replay_rows={replay_rows_seen} total_rows={rows_seen} "
        ...
        epoch_record = {
            ...
            "training_primary_rows": primary_rows_seen,
            "training_replay_rows": replay_rows_seen,
    ...
    manifest = {
        ...
        "replay": (
            {
                "path": str(args.replay_train.resolve()),
                "sha256": _sha256(args.replay_train),
                "available_rows": replay_available_rows,
                "planned_rows_per_epoch": planned_replay_rows,
                "used_rows": sum(
                    int(record.get("training_replay_rows", 0))
